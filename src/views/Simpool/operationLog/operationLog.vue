<template>
  <div>
    <v-card>
      <myTabel :tableData=data></myTabel>
    </v-card>
    <v-navigation-drawer
      :value="drawerRight"
      fixed
    ...
    <v-container  grid-list-md>
      ...
      <v-layout row wrap>
        ...
        <v-flex  d-flex xs12 md12>
          <v-text-field  :counter="10" label="日志时间" required ></v-text-field>
        </v-flex>
        <v-flex  d-flex xs12 md12>
          <v-text-field  :counter="10" label="提取人" required ></v-text-field>
        </v-flex>
        <v-combobox :items="items"  label="状态" ></v-combobox>
        <v-flex d-flex xs12 md12>
        </v-flex>
        <v-footer  app d-flex>
          <v-layout row wrap>
